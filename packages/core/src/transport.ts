...
import { Context, DOM_EVENT, jsonStringify, noop, objectValues, deepMerge } from './utils'
...
export class Batch<T> {
  ...
  private process(message: T) {
    const contextualizedMessage = deepMerge({}, this.contextProvider(), message) as Context
